...
int main() {
    try {
        ...
        main_queue.enqueue([&inBufDev] {inBufDev.sync(XCL_BO_SYNC_BO_TO_DEVICE, BUFFER_BYTES, 0); });
        ...
        main_queue.enqueue([&run_kernel] { run_kernel.start(); run_kernel.wait();});


        // Get output from device
        xrt::queue::event out_bo_event = main_queue.enqueue([&outBufDev] {
        ...
        for (int i = 0; i < BUFFER_BYTES / sizeof(int32_t); ++i) {
            std::cout << "Data[" << i + 1 << "]: " << out_data[i] << std::endl;
        }
